...
def test_init(var, sig, output, monkeypatch, capsys, **kwargs):
    ...
    monkeypatch.setattr(user_cli, 'verify_token',  )
    monkeypatch.settattr(user_cli, 'string_to_dict',  )
    monkeypatch.settattr(user_cli, 'query_request',  )
    monkeypatch.settattr(user_cli, 'print_results',  )

    def mock_input(x):
        if x == "Please ebter your secret token or enter nothing to continue as not registered user: ":
            return token
