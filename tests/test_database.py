...
def test_parse_statement_user(demo_db_path):
    ...
    output0 = conn.parse_statement('SELECT chr, pos, ref, alt FROM variants WHERE chr = ? AND pos = ? AND ref = ? AND pos = ?;',parameters0)
    parameters1 = ("X",10000000, "C", "G")
    output1 = conn.parse_statement('SELECT chr, pos, ref, alt FROM variants WHERE chr = ? AND pos = ? AND ref = ? AND pos = ?;', parameters1)
