...
def test_parse_statement_user(demo_db_path):
    conn = database.ConnectDatabase(demo_db_path)
    output0 = conn.parse_statement("SELECT chr, pos, ref, alt FROM variants WHERE chr = '1' AND pos = 10000000 AND ref = 'C' AND pos = 'G';")
    output1 = conn.parse_statement("SELECT chr, pos, ref, alt FROM variants WHERE chr = 'X' AND pos = 10000000 AND ref = 'C' AND pos = 'G';")
