...
@app.route(
    "/query", methods=["POST"]
)  # change user_cli to make POST instead of GET request : done
def get_api():  # gets json/dict as POST request : done
    ...
    if auth == 0:
        ...
        ann_var = connectDb.handle_request(
            var
...
def request_permission(token):
    ...
    auth = con.parse_statement(
        "SELECT authorization,count_req FROM login WHERE token = ?", [token]
    )
    if auth[1] > 10:
        return 0
    else:
        con.parse_statement(
            "UPDATE login SET count_req = count_req + 1 WHERE token = ?", [token]
        )
        return auth[0]
